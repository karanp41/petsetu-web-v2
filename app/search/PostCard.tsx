...
export function PostCard({ post }: PostCardProps) {
  ...
  // Format posted date from ISO string to a nice readable form like "Sep 21, 2025"
  let postedDate: string | null = null;
  if (post.createdAt) {
    try {
      postedDate = format(parseISO(post.createdAt), "PP");
    } catch (e) {
      postedDate = null;
    }
  }
  ...
  return (
    <Card className="overflow-hidden group border-0 shadow hover:shadow-lg transition-all duration-300 focus-within:ring-2 focus-within:ring-orange-500">
      <Link
        href={`/post/${post._id}`}
